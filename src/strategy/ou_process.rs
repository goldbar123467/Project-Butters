...
//! - σ (sigma): Volatility
//!
//! The OU process follows: dX(t) = θ(μ - X(t))dt + σdW(t)
//!
//! Entry signals are generated when price deviates significantly from equilibrium,
//! measured by z_ou = (log_price - μ) / (σ / sqrt(2θ))
...
impl OUProcess {
    ...
    fn estimate_params(&self) -> Option<OUParams> {
        ...
        // Var(X) = σ² / (2θ) for stationary OU process
        // Therefore: σ = sqrt(Var(X) * 2θ)
        ...
        // Calculate half-life: t_1/2 = ln(2) / θ
    ...
    fn generate_signal(&self, current_log_price: f64) -> OUSignal {
        match &self.params {
            Some(params) if params.is_valid() => {
                ...
                // Use 3.5σ threshold for initial signal classification
