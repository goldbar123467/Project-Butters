...
pub struct MeanReversionStrategy {
    ...
    /// Daily P&L tracking
    daily_pnl: f64,
    /// Consecutive bars in oversold territory (z < -threshold)
    consecutive_oversold: usize,
    /// Consecutive bars in overbought territory (z > +threshold)
    consecutive_overbought: usize,
}
...
impl MeanReversionStrategy {
    ...
    pub fn update(&mut self, price: f64) -> Option<TradeAction> {
        ...
        // Track consecutive bars above/below threshold (for sniper mode)
        if zscore_result.is_oversold(self.config.z_threshold) {
            self.consecutive_oversold += 1;
            self.consecutive_overbought = 0; // Reset opposite counter
        } else if zscore_result.is_overbought(self.config.z_threshold) {
            self.consecutive_overbought += 1;
            self.consecutive_oversold = 0; // Reset opposite counter
        } else {
            // Z-score back in normal range - reset both counters
    ...
    fn evaluate_action(&self, zscore: &ZScoreResult, current_price: f64) -> TradeAction {
        let bars_required = self.config.consecutive_bars_required;

        match self.position {
            PositionState::Flat => {
                // Look for entry signals - require consecutive bars above threshold (sniper mode)
                if zscore.is_oversold(self.config.z_threshold)
                    && self.consecutive_oversold >= bars_required {
                    TradeAction::EnterLong
                } else if zscore.is_overbought(self.config.z_threshold)
                    && self.consecutive_overbought >= bars_required {
    ...
    fn on_trade_executed(&mut self, action: TradeAction, price: f64) {
        match action {
            TradeAction::EnterLong => {
                ...
                self.daily_trades += 1;
                // Reset consecutive counters after entry
                self.consecutive_oversold = 0;
                self.consecutive_overbought = 0;
            }
            TradeAction::EnterShort => {
